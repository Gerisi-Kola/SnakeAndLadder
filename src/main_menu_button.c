...
int main_menu_button_init(SDL_Texture *texture_button[], SDL_Renderer *renderer){
    ...
    if (*texture_button != NULL) {
        printf("main_menu_button_init : texture_button != NULL\n");
        for (int i = 0; i < 4; i++){
            SDL_QueryTexture(*texture_button, NULL, NULL, &rect_button[i].w, &rect_button[i].h);
            SDL_RenderCopy(renderer, *texture_button, NULL, &rect_button[i]);
